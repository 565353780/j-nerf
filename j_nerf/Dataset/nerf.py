...
class NerfDataset:
    ...
    def load_data(self, root_dir=None):
        ...
        def read_focal_length(resolution: int, axis: str):
            if "fl_" + axis in json_data:
                ...
                return fov_to_focal_length(
                    resolution, json_data["camera_angle_" + axis] * 180 / pi
                )
            else:
                return 0
        ...
        aabb_range = (0.5, 0.5)
        self.aabb_range = (
            aabb_range[0] - self.aabb_scale / 2,
            aabb_range[1] + self.aabb_scale / 2,
        )
        self.H = int(self.H)
        self.W = int(self.W)
        ...
        if self.img_alpha and self.image_data.shape[-1] == 3:
            self.image_data = jt.concat(
                [self.image_data, jt.ones(self.image_data.shape[:-1] + (1,))], -1
            ).stop_grad()
    ...
    def generate_random_data(self, index, bs):
        ...
        rays_d = jt.concat(
            [(xy - principal_point) * res / focal_length, jt.ones([bs, 1])], dim=-1
        )
    ...
    def generate_rays_total(self, img_id, H, W):
        ...
        xy = (
            jt.stack(
                jt.meshgrid(
                    (jt.linspace(0, H - 1, H) + 0.5) / H,
                    (jt.linspace(0, W - 1, W) + 0.5) / W,
                ),
                dim=-1,
            )
            .permute(1, 0, 2)
        ...
        rays_d = jt.concat(
            [(xy - principal_point) * res / focal_length, jt.ones([H * W, 1])], dim=-1
        )
        rays_d = jt.normalize(xforms[:, :3].matmul(rays_d.unsqueeze(2)))
        rays_d = rays_d.squeeze(-1)
    ...
    def generate_rays_total_test(self, img_ids, H, W):
        ...
        focal_length = jt.gather(self.focal_lengths, 0, img_ids)
        xforms = jt.gather(self.transforms_gpu, 0, img_ids)
        principal_point = jt.gather(self.metadata[:, 4:6], 0, img_ids)
        ...
        xy_int = (
            jt.stack(
                jt.meshgrid(jt.linspace(0, H - 1, H), jt.linspace(0, W - 1, W)), dim=-1
            )
            .permute(1, 0, 2)
            .reshape(-1, 2)
        )
        ...
        rays_d = jt.concat(
            [
                (xy - jt.gather(principal_point, 0, img_ids)) * res / focal_length,
                jt.ones([H * W, 1]),
            ],
            dim=-1,
        )
        rays_d = jt.normalize(
            jt.gather(xforms, 0, img_ids)[:, :, :3].matmul(rays_d.unsqueeze(2))
        )
        # resolution W H
        # img H W
