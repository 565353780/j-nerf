...
template <typename T> TCNN_HOST_DEVICE T div_round_up(T val, T divisor) {
  return (val + divisor - 1) / divisor;
}

template <typename T> struct PitchedPtr {
  TCNN_HOST_DEVICE PitchedPtr() : ptr{nullptr}, stride_in_bytes{sizeof(T)} {}
  TCNN_HOST_DEVICE PitchedPtr(T *ptr, size_t stride_in_elements,
                              size_t offset = 0)
      : ptr{ptr + offset},
        stride_in_bytes{(uint32_t)(stride_in_elements * sizeof(T))} {}
...
__device__ uint32_t grid_index(const uint32_t feature,
...
// The second part of the loop condition is needed to avoid integer overflows in
// finer levels.
#pragma unroll
...
__device__ inline float identity_derivative(float val) { return 1; }
template <uint32_t N_FLOATS> using vector_fullp_t = vector_t<float, N_FLOATS>;
...
kernel_grid(const uint32_t num_elements, const uint32_t num_grid_features,
            ...
            const uint32_t base_resolution, const float log2_per_level_scale,
            const float quantize_threshold, float max_level,
            const uint32_t interpolation_type, const uint32_t grid_type,
            const T *__restrict__ grid, const float *__restrict__ positions_in,
  ...
  const uint32_t hashmap_size =
      hashmap_offset_table[level + 1] - hashmap_offset_table[level];
  const float scale =
      exp2f(level * log2_per_level_scale) * base_resolution - 1.0f;
...
#pragma unroll
  for (uint32_t dim = 0; dim < N_POS_DIMS; ++dim) {
    ...
    pos_fract(positions_in[i + dim * num_elements], &pos[dim],
              &pos_derivative[dim], &pos_grid[dim], scale, identity_fun,
              identity_derivative);
  }

  auto grid_val = [&](const uint32_t local_pos[N_POS_DIMS]) {
    uint32_t index = grid_index<N_POS_DIMS, N_FEATURES_PER_LEVEL>(
        0, hashmap_size, grid_resolution, local_pos);
...
#pragma unroll
      ...
      *(vector_fullp_t<N_FEATURES_PER_LEVEL>
            *)&dy_dx[i * fan_out_grad + level * N_FEATURES_PER_LEVEL +
                     grad_dim * num_grid_features] = grad;
    }
  }
}
...
template <typename T>
__global__ void
transpose_encoded_position(const uint32_t n_elements,
...
template <typename T>
__global__ void transpose_gradients(const uint32_t n_elements,
...
__global__ void kernel_grid_backward(
    ...
    const uint32_t *hashmap_offset_table, const uint32_t base_resolution,
    const float log2_per_level_scale, float max_level,
    const bool stochastic_interpolation, const uint32_t interpolation_type,
    const uint32_t grid_type, GRAD_T *__restrict__ grid_gradient,
  ...
  auto add_grid_gradient = [&](const uint32_t local_pos[N_POS_DIMS],
                               ...
                               const float weight) {
    uint32_t index = grid_index<N_POS_DIMS, N_FEATURES_PER_LEVEL>(
        feature, hashmap_size, grid_resolution, local_pos);
    // // #if TCNN_MIN_GPU_ARCH >= 60 // atomicAdd(__half2) is only supported
    // with compute capability 60 and above
...
#pragma unroll
    for (uint32_t dim = 0; dim < N_POS_DIMS; ++dim) {
      pos_fract(positions_in[i + dim * num_elements], &pos[dim], &pos_grid[dim],
                scale, identity_fun);
    }
  }

  auto grad = dL_dy[(i + level * num_elements) * (N_FEATURES_PER_LEVEL) /
                        N_FEATURES_PER_THREAD +
                    feature / N_FEATURES_PER_THREAD];
