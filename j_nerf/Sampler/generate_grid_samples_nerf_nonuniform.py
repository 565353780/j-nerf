...
class generate_grid_samples_nerf_nonuniform(Function):
    ...
    def execute(
        self, density_grid, n_elements, density_grid_ema_step, max_cascade, thresh
    ) -> None:
        ...
        self.thresh = thresh
        output = jt.code(
            shapes=[(self.n_elements, 3), (self.n_elements,)],
            dtypes=[density_grid.dtype, jt.int32],
            inputs=[density_grid, density_grid_ema_step],
            cuda_header=global_headers
            + self.density_grad_header
            + '#include "generate_grid_samples_nerf_nonuniform.h"',
            cuda_src=f"""
