...
class DensityGridSampler(nn.Module):
    def __init__(self, update_den_freq=16, update_block_size=5000000):
        ...
        if self.dataset.aabb_scale > max_aabb_scale:
            ...
            print(
                f"""Warning:Default max value of NeRF dataset's aabb_scale is {max_aabb_scale}, but now is {self.dataset.aabb_scale}.
        ...
        self.density_grid_bitfield_n_elements = (
        ...
        header_path = "../j-nerf/j_nerf/Cpp/density_grid_sampler/"
        ...
        if self.const_dt:
            ...
            """
        self.density_grad_host_header = self.density_grad_header.replace(
        ...
        self.generate_grid_samples_nerf_nonuniform = (
            generate_grid_samples_nerf_nonuniform(
                self.density_grad_header, aabb_range=self.aabb_range
            )
        )
        self.splat_grid_samples_nerf_max_nearest_neighbor = (
            splat_grid_samples_nerf_max_nearest_neighbor(
                self.density_grad_header,
        ...
        self.update_bitfield = update_bitfield(self.density_grad_header)
        self.rays_sampler = RaySampler(
            self.density_grad_host_header,
            self.near_distance,
            self.cone_angle_constant,
            self.aabb_range,
    ...
    def sample(self, img_ids, rays_o, rays_d, rgb_target=None, is_training=False):
        ...
        (
            ...
            rays_index,
            rays_numsteps,
            rays_numsteps_counter,
        ) = self.rays_sampler.execute(
            rays_o=rays_o,
            rays_d=rays_d,
            density_grid_bitfield=self.density_grid_bitfield,
            metadata=self.dataset.metadata,
            imgs_id=img_ids,
            xforms=self.dataset.transforms_gpu,
        )
    ...
    def update_density_grid_nerf(
        self,
        decay: float,
        n_uniform_density_grid_samples: int,
        n_nonuniform_density_grid_samples: int,
    ):
        ...
        (
            density_grid_positions_uniform,
            density_grid_indices_uniform,
        ) = self.generate_grid_samples_nerf_nonuniform.execute(
            self.density_grid,
            n_uniform_density_grid_samples,
            self.density_grid_ema_step,
            self.max_cascade,
            -0.01,
        )
        (
            density_grid_positions_nonuniform,
            density_grid_indices_nonuniform,
        ) = self.generate_grid_samples_nerf_nonuniform.execute(
            self.density_grid,
        ...
        self._density_grid_positions = self._density_grid_positions.reshape(-1, 3)
        with jt.no_grad():
        ...
        self.density_grid_tmp = (
            self.splat_grid_samples_nerf_max_nearest_neighbor.execute(
                self._density_grid_indices,
                self._mlp_out,
                self.density_grid_tmp,
                n_density_grid_samples,
            )
        ...
        (
            self.density_grid_bitfield,
            self.density_grid_mean,
        ) = self.update_bitfield.execute(
    ...
    def update_batch_rays(self):
        ...
        rays_per_batch = int(
            self.n_rays_per_batch * self.target_batch_size / measured_batch_size
        )
