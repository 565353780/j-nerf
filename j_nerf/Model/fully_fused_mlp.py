...
class FullyFusedMlp_weight(jt.Function):
    def __init__(self, weights, check_mid="0", output_activation="Activation::None"):
        ...
        con_weights = []
        self.code_path = "../j-nerf/j_nerf/Cpp/code_ops/"
        self.so_name = self.code_path + "fully_fused_mlp_function.o"
            ...
            if i == len(weights) - 1:
                ...
                if weights[i].shape[1] < 16:
                    weights[i] = jt.concat(
                        [
                            weights[i],
                            jt.zeros((weights[i].shape[0], 16 - weights[i].shape[1])),
                        ],
                        -1,
                    ).float16()
    ...
    def execute(self, a, con_weights):
        ...
        if self.padded_shape0 > 0:
            self.padded_input = jt.concat(
                [a, jt.zeros((self.padded_shape0, a.shape[1])).float16()], 0
            )
        else:
            self.padded_input = self.input
        self.outputs, self.output_intermediate = jt.code(
            [
                (self.padded_input.shape[0], 16),
                (self.padded_input.shape[0] * (len(self.weights) - 1), self.width),
            ],
            [a.dtype, a.dtype],
            [self.padded_input, con_weights],
            cuda_header=cuda_header,
            cuda_src=cuda_src,
        )
        self.outputs.compile_options = {
            f"FLAGS: -I{self.code_path} -Xlinker {self.so_name} ": 1
        }
    ...
    def grad(self, grads):
        if self.padded_shape0 != 0:
            grads = jt.concat(
                [grads, jt.zeros((self.padded_shape0, grads.shape[1])).float16()], 0
            )
        self.grads = grads
        ...
        """
        output, grad_temps = jt.code(
            [
                (self.padded_input.shape[0], self.input.shape[1]),
                ((len(self.weights) - 1) * self.padded_input.shape[0], self.width),
            ],
            [self.input.dtype, self.input.dtype],
            [grads.transpose(), self.con_weights, self.output_intermediate],
            cuda_header=cuda_header,
            cuda_src=cuda_src,
        )
        ...
        if not need_last:
            output = jt.matmul(
                grad_temps[self.padded_input.shape[0] * (len(self.weights) - 2) :],
                self.con_weights[: self.first_stride].reshape(
                    self.width, self.weight_shape0
                ),
            )
        wt = []
        for i in range(len(self.weights)):
            if i == 0:
                if os.environ.get("FUSE_TRANSPOSE", "0") == "1":
                    new_weight = jt.cublas.ops.cublas_acc_matmul(
                        grad_temps[
                            self.padded_input.shape[0] * (len(self.weights) - 2 - i) :
                        ],
                        self.padded_input,
                        1,
                        0,
                ...
                if os.environ.get("FUSE_TRANSPOSE", "0") == "1":
                    new_weight = jt.cublas.ops.cublas_acc_matmul(
                        ...
                        self.output_intermediate[
                            self.padded_input.shape[0] * (len(self.weights) - 2) :
                        ],
                        1,
                        0,
                    )
                ...
                if os.environ.get("FUSE_TRANSPOSE", "0") == "1":
                    new_weight = jt.cublas.ops.cublas_acc_matmul(
                        grad_temps[
                            self.padded_input.shape[0]
                            * (len(self.weights) - 2 - i) : self.padded_input.shape[0]
                            * (len(self.weights) - 1 - i)
                        ],
                        self.output_intermediate[
                            self.padded_input.shape[0]
                            * (len(self.weights) - 2 - i) : self.padded_input.shape[0]
